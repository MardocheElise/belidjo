...
export default function ProductsPage({ selectedCategory = "" }: ProductsPageProps) {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      ...
      try {
        ...
        // ✅ FILTRER UNIQUEMENT LES PRODUITS ACTIFS côté frontend
        const activeProducts = data.products.filter((product: Product) => product.isActive === true);
        ...
        console.log(`✅ ${sortedProducts.length} produits actifs après filtrage et tri`);
  ...
  if (products.length === 0) {
    return (
      <div className="p-8 text-center">
        <p className="text-gray-500 text-lg">
          {selectedCategory 
            ? `Aucun produit actif trouvé dans la catégorie "${selectedCategory}"`
  ...
  return (
    <div className="p-1 w-full max-w-6xl">
      ...
      {/* Grille de produits - N'affiche QUE les produits actifs */}
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
        {products.map((product) => (
          <div key={product._id} className="border rounded-lg p-4 shadow-md hover:shadow-xl transition-shadow duration-300 bg-white">
            <div className="relative group">
